        ...
        children: [
          ...
          Expanded(
            child: ListView.builder(
              itemCount: listaDesempenho.length,
              itemBuilder: (context, i) {
                Desempenho item = listaDesempenho[i];


                return Card(
                  margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 6),
                  child: ListTile(
                    leading: Icon(Icons.bar_chart, color: Color(0xFFC8B7ED)),
                    title: Text(item.materia),
                    subtitle: Text("Acertos: ${item.acertos} de ${item.total}"),
                  ),
                );
              },
            ),
